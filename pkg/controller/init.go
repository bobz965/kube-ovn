...
func (c *Controller) InitIPAM() error {
	...
	for _, subnet := range subnets {
		...
		if subnet.Status.U2OInterconnectionIP != "" {
			if _, _, _, _, err = c.ipam.GetStaticAddress(u2oInterconnName, u2oInterconnLrpName, subnet.Status.U2OInterconnectionIP, nil, subnet.Name, true); err != nil {
	...
	for _, ip := range ips {
		...
		if _, _, _, _, err = c.ipam.GetStaticAddress(ipamKey, ip.Name, ip.Spec.IPAddress, &ip.Spec.MacAddress, ip.Spec.Subnet, true); err != nil {
	...
	for _, pod := range pods {
		...
		for _, podNet := range podNets {
			if pod.Annotations[fmt.Sprintf(util.AllocatedAnnotationTemplate, podNet.ProviderName)] == "true" {
				...
				_, _, _, _, err := c.ipam.GetStaticAddress(key, portName, ip, &mac, podNet.Subnet.Name, true)
				if err != nil {
	...
	for _, vip := range vips {
		...
		if _, _, _, _, err = c.ipam.GetStaticAddress(ipamKey, vip.Name, vip.Status.V4ip, &vip.Status.Mac, vip.Spec.Subnet, true); err != nil {
	...
	for _, eip := range eips {
		...
		if _, _, _, _, err = c.ipam.GetStaticAddress(eip.Name, eip.Name, eip.Status.IP, &eip.Spec.MacAddress, externalNetwork, true); err != nil {
	...
	for _, oeip := range oeips {
		if _, _, _, _, err = c.ipam.GetStaticAddress(oeip.Name, oeip.Name, oeip.Status.V4Ip, &oeip.Status.MacAddress, oeip.Spec.ExternalSubnet, true); err != nil {
	...
	for _, node := range nodes {
		if node.Annotations[util.AllocatedAnnotation] == "true" {
			...
			_, v4IP, v6IP, _, err := c.ipam.GetStaticAddress(portName, portName,
